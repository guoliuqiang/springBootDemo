...
function extractEntries(tabs) {
  return keyBy(flattenDeep(map(tabs, 'entries')), 'id');
}
...
function PropertiesPanel(config, eventBus, modeling, propertiesProvider, commandStack, canvas) {
  ...
  if (config.data) {
    let assetOptions = config.data.assets ? config.data.assets.map((x) => {
      return {value: x.assetCode, name: x.name, data: x};
    }) : [];
    let channelOptions = config.data.channels ? config.data.channels.map((x) => {
      let data = x.pages.map((y) => {
        let fields = y.fields.map((z) => {
          return {value: z.fieldId, name: z.fieldName}
        })
        return {value: y.pageId, name: y.pageName, data: fields}
      })
      return {value: x.channelType, name: x.channelName, data: data};
    }) : [];
    let resourceTypeOptions = config.data.resources ? config.data.resources.map((x) => {
      let resources = x.resources.map((z) => {
        return {value: z.id, name: z.name}
      })
      return {value: x.resourceType, name: x.resourceType, data: resources};
    }) : [];
    let messageTypeOptions = config.data.messages ? config.data.messages.map((x) => {
      let fields = x.messageRefDtoList.map((y) => {
        return {value: y.refId, name: y.refName}
      })
      return {value: x.messageType, name: x.messageTypeName, data: fields}
    }) : [];

    //标签处理
    let taglistOptions = config.data.tags ? config.data.tags.map((x) => {
      return {value: x.id, name: x.tagname, data:x}
    }) : [];
...
PropertiesPanel.prototype._init = function(config) {
  ...
  eventBus.on('propertiesPanel.showCreateResourceDialog', function(e) {
    var node = e.node;
...
PropertiesPanel.prototype.update = function(element) {
  ...
  // (2) Get tabs
  var tabs = this._propertiesProvider.getTabs(element, this._data);
...
PropertiesPanel.prototype.setData = function(data) {
  ...
  if(this.selectedElement) {
    for (var key in data) {
      ...
      let entryNode = domClosest(this.selectedNode, '[data-entry]');
      var command = entry.set(this.selectedElement, data, entryNode);
      ...
      if (commandToExecute) {
        this._commandStack.execute(commandToExecute.cmd, commandToExecute.context || { element : this.selectedElement });
      } else {
...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  forEach(current.tabs, function(group) {
...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  var panelNode = domify('<div class="bpp-properties"></div>'),
      ...
      '</div>'),
      tabContainerNode = domify('<div class="bpp-properties-tabs-container el-form el-form--inline"></div>');

  panelNode.appendChild(headerNode);

  forEach(tabs, function(group) {

    if (!group.id) {
      throw new Error('group must have an id');
    }

    var groupNode = domify('<div data-group="' + escapeHTML(group.id) + '"></div>');
    ...
    forEach(group.entries, function(entry) {
      ...
      var entryNode = domify('<div class="el-form-item el-form-item--small" data-entry="' + escapeHTML(entry.id) + '"></div>');
    ...
    tabContainerNode.appendChild(groupNode);
  });
